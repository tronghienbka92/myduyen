...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        #endregion
        public virtual void ExportdtdiemToXlsx(Stream stream, List<DoanhThuChang> DtChang, List<DoanhThuDiem> Dtdiem, DateTime NgayDi, string BienSo)
        {
            if (stream == null)
                throw new ArgumentNullException("stream");
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                // get handle to the existing worksheet
                var worksheet = xlPackage.Workbook.Worksheets.Add("DT Chang");
                //Create Headers and format them
                worksheet.DefaultRowHeight = 20;
                worksheet.DefaultColWidth = 20;
                worksheet.Cells[1,1,1,2].Merge = true;

                worksheet.Cells[1, 1].Value = "Doanh thu theo chặng";

                worksheet.Cells[2,1].Value = "Biển số : " + BienSo;
                ...
                worksheet.Cells[2,2].Value = "Ngày " + NgayDi.ToString("dd/MM/yyyy HH:mm");
                worksheet.Cells[2, 2].Style.Font.Bold = true;

                worksheet.Cells[4,1,5,1].Merge = true;
                worksheet.Cells[4,1].Value = "Tên chặng";

                worksheet.Cells[4,2,5,2].Merge = true;
                worksheet.Cells[4,2].Value = "Số khách";

              

                int row = 6;
                int stt = 1;
                foreach (var item in DtChang)
                {
                    int col = 1;
                    worksheet.Cells[row, col].Value = item.TenChang;
                    col++;
                ...
                var modelTableheader = worksheet.Cells[4,1,5,2];
                ...
                var modelTable = worksheet.Cells[6,1,row,2];
                // Assign borders
                modelTable.Style.Border.Top.Style = ExcelBorderStyle.Dashed;
                modelTable.Style.Border.Left.Style = ExcelBorderStyle.Thin;
                modelTable.Style.Border.Right.Style = ExcelBorderStyle.Thin;
                ...
                worksheet.Cells[row, 1, row, 3].Value = "Doanh thu theo điểm";
                ...
                row++;
                int rowstart = row;
                worksheet.Cells[rowstart, 1, rowstart+1, 1].Merge = true;
                worksheet.Cells[rowstart, 1].Value = "Tên điểm";

                worksheet.Cells[rowstart, 2, rowstart+1, 2].Merge = true;
                worksheet.Cells[rowstart, 2].Value = "Số khách lên";

                worksheet.Cells[rowstart, 3, rowstart + 1, 3].Merge = true;
                worksheet.Cells[rowstart, 3].Value = "Số khách xuống";
                row = row + 2;
                int rowbody = row;
                foreach (var item in Dtdiem)
                {
                    int col = 1;
                    worksheet.Cells[row, col].Value = item.TenDiem;
                    col++;
                    worksheet.Cells[row, col].Value = item.SoKhachLen;
                    col++;
                    worksheet.Cells[row, col].Value = item.SoKhachXuong;
                    col++;
                    row++;
                   
                }
                var modelTableheader2 = worksheet.Cells[rowstart, 1, rowstart+1, 3];
                modelTableheader2.Style.Border.Top.Style = ExcelBorderStyle.Thin;
                modelTableheader2.Style.Border.Left.Style = ExcelBorderStyle.Thin;
                modelTableheader2.Style.Border.Right.Style = ExcelBorderStyle.Thin;
                modelTableheader2.Style.Border.Bottom.Style = ExcelBorderStyle.Thin;
                modelTableheader2.Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                modelTableheader2.Style.Font.Bold = true;
                //style body

                var modelTable2 = worksheet.Cells[rowbody, 1, row, 3];
                // Assign borders
                modelTable2.Style.Border.Top.Style = ExcelBorderStyle.Dashed;
                modelTable2.Style.Border.Left.Style = ExcelBorderStyle.Thin;
                modelTable2.Style.Border.Right.Style = ExcelBorderStyle.Thin;
                modelTable2.Style.Border.Bottom.Style = ExcelBorderStyle.DashDot;
